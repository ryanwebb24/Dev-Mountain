...
test("can search Google twice", async () => {
  ...
  await driver.findElement(By.name("q")).sendKeys("anime", Key.RETURN);
  ...
  await driver.findElement(By.name("q")).sendKeys("fortnite", Key.RETURN);
  // TODO Wait for the results page to load
  await driver.wait(until.titleIs("fortnite - Google Search"), 1000); 
});
