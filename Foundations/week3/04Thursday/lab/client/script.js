...
sayHelloButton.addEventListener('mouseover', mouseEnter)
// PROBLEM 3
/*
...
sayHelloButton.addEventListener('mouseout', mouseLeave)
// PROBLEM 4
/*
...
const ohMy = () => {
    ...
    .then(response => {
        for(let i in response.data){
            ...
            pElement.innerHTML = response.data[i]
...
document.getElementById('query-button').addEventListener('click', queryBtn)
...
/*
    ...
    if you send no query then it will say you sent an empty query
    2: Send more than 1 query on the URL -- what happened?
    sends an object with a message and then both queries you sent
*/
...
formFood.addEventListener('submit', foodList)