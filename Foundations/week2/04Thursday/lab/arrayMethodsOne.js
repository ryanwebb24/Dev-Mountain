...
const postTaxPrices = prices.map(
  (element) => Math.round(element * 1.07 * 100) / 100
);
...
// CODE HERE
const salesTaxTotal = orders
  .map((element) => Math.round(element.price * (1 + element.tax) * 100) / 100)
...
const bobsTotal = purchases
  .filter((element) => element.owner.toLowerCase().includes("bob"))
